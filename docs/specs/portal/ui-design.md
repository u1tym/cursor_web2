...
色・余白・部品の見た目、画面分割とナビは、ログイン画面とメニュー画面では `15-ui-style.mdc` の殻を使わない。配置は本ファイルに従う。他機能の画面は参照しない。
...
| メイン | カード | 機能 1 件につき 1 枚。機能アイコンとタイトルを示す。押すとその機能の遷移先へ進む。 |
...
| 部品 | 必須 | 初期値 | バリデーション | 備考 |
|------|------|--------|----------------|------|
| カード（機能） | - | 割当なしなら 0 件 | - | 表示順どおり。アイコンはデータとして得た画像。ディスク上のファイルは使わない |
