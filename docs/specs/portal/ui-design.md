...
|--------|--------|------|------|----------|
| SCR-001 | ログイン | `/portal/login` | ユーザ名とパスワードでログインする。 | REQ-006, REQ-007, REQ-009 |
| SCR-002 | メニュー | `/portal/menu` | 割り当てられた機能を示し、遷移とログアウトを行う。 | REQ-004, REQ-005, REQ-006, REQ-008, REQ-012, REQ-013 |

パス `/portal/` は画面を持たず、ログイン中なら SCR-002、未ログインなら SCR-001 へ進む。
...
ヘッダとナビは置かない。画面中央に、ロゴ、ユーザ名、パスワード、ログインボタンを縦に置く。

| 領域 | 部品 | 内容・動作 |
|------|------|------------|
| メイン | ロゴ | ポータルのロゴ。入力欄の上に示す。押しても遷移しない。 |
...
| 起点 | 操作 | 条件 | 行き先 |
|------|------|------|--------|
| `/portal/` | 開く | 未ログイン | SCR-001 |
| `/portal/` | 開く | ログイン中 | SCR-002 |
...
| 2026-08-29 17:15 | 未承認 | 公開パスを `/portal/login` と `/portal/menu` にする |
| 2026-08-29 19:19 | 承認済み | 公開パス `/portal/login` と `/portal/menu` の改訂を承認 |
