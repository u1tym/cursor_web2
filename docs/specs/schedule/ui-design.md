...
| ナビのカテゴリ | ナビの月の下（PC）。スマートフォンはナビの「Categories」 | PC はカテゴリ一覧。スマートフォンはパネルを開く。 |
...
| 休日の登録ダイアログ | モーダル | ヘッダ「Settings」から出す。ユーザ休日の一覧・追加。行を選ぶと休日入力（編集）。 |
...
| ツールバー | 見出し | 表示対象月。`Month YYYY`（例: `August 2026`）。 |
| ツールバー | テキストボタン | 「Prev」。表示対象月を 1 か月前にする。 |
| ツールバー | テキストボタン | 「Next」。表示対象月を 1 か月後にする。 |
| ツールバー | 副ボタン | 「Today」。表示対象月を操作している日の年月にする。 |
| ツールバー | 選択 | 週の開始。値は「Starts Sunday」「Starts Monday」。変更は表示設定として保存する。 |
| ナビ | カテゴリ一覧 | 各行は色の見本、名称、表示／非表示の切替。未削除は名称変更・色変更・削除ができる。行を選ぶと表示／非表示を切り替える。 |
...
| 日セル | Caption | 日付。セル左上。土曜日は `--color-primary`。日曜日、日本の祝日、ユーザ休日は `--color-danger`。土曜かつ休日は `--color-danger`。表示対象月以外の前後の日も同じ色規則。月外であることは枠や濃度で区別してよいが、独自色は置かない。 |
| 日セル | Caption | 日本の祝日名称・ユーザ休日の名称（あるとき）。日付と同じ行の右上。文字色は `--color-danger`。両方ある日は両方出す。無いときは出さない。 |
...
ナビ: Schedule（見出し）／Categories／月 5 件。
...
| メイン | エラー | 失敗時、領域先頭に一文。内部理由は出さない。 |
| メイン | 成功 | 保存・削除の成功時、領域先頭に一文。数秒で消す。 |
| ツールバー | 見出し | 表示対象月。`Month YYYY`。 |
| ツールバー | テキストボタン | 「Prev」「Next」。 |
| ツールバー | 副ボタン | 「Today」。表示対象月を操作している日の年月にし、その日を選んだ状態にして詳細を出す。 |
| ツールバー | 選択 | 週の開始。「Starts Sunday」「Starts Monday」。表示設定として保存する。 |
...
休日の登録ダイアログ（ヘッダ「Settings」。PC / スマートフォン共通）:
...
| 設定 | ヘッダ「Settings」 | 休日の登録ダイアログを出す |
| 月へ切替 | ナビの `Mon YYYY` | 表示対象月をその年月にする。ナビの月 5 件を、その月が中央になるよう更新する。カレンダーを読み直す |
| 前月 / 翌月 | 「Prev」「Next」 | 表示対象月を変える。カレンダーを読み直す |
| 今日へ戻る | 「Today」 | 表示対象月を操作している日の年月にする。スマートフォンではその日を選び詳細を出す |
...
- PC: ナビにカテゴリ一覧、コンテンツに月カレンダーを同時に示す。ユーザ休日はヘッダ「Settings」の登録ダイアログで操作する。日セルにスケジュール本文を出す。日付選択は新規追加。TODO の実施状態は日セルのチェックで変えられる。`＋N` のマウスオーバーで残件を出す。
- スマートフォン: カテゴリはナビの「Categories」、休日はヘッダ「Settings」からダイアログで操作する。カレンダーは日付と点。詳細は日付選択後に同一画面の下へ出す（別パスへは進まない）。ページ全体はスクロールせず、詳細欄の中をスクロールする。新規追加は右下の「＋」。詳細から実施状態は変えない。
...
| SCR-001 | ヘッダ「Settings」 | なし | 休日の登録ダイアログを出す。パスは変えない |
...
| REQ-021 | PC のナビのカテゴリとカレンダー。休日はヘッダ「Settings」の登録ダイアログ |
