...
- `ui-design.md` / SCR-001（日のメニュー、休日入力）
...
未削除の休日を年月日順で取得する。日セルの右クリック（スマートフォンは長押し）でメニューを出す。ユーザ休日でなければ名称入力のあと追加。ユーザ休日ならその場で論理削除。設定ダイアログに休日一覧は置かない。空は送らない。保存・削除後にカレンダーの名称と日付色を更新する。

### 完了条件

- [ ] 日の右クリックでユーザ休日の追加・削除ができる
- [ ] 設定ダイアログに祝日の一覧・追加が無い
- [ ] カレンダーに名称が出て、日付が赤になる
