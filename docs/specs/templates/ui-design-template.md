...
部品名、画面タイプ、状態の見せ方、操作ラベル、見た目は `.cursor/rules/15-ui-style.mdc` に従う。他機能の画面は参照しない。
...
| 画面ID | 画面名 | パス | 画面タイプ | 目的 | 対応 REQ |
|--------|--------|------|------------|------|----------|
| SCR-001 | ＜画面名＞ | `/＜path＞` | 一覧 / 詳細 / 入力 / 確認 | ＜役割＞ | REQ-001 |
...
`15-ui-style.mdc` の画面タイプに沿って、領域ごとに部品を書く。部品名は同ルールのカタログの語を使う。
